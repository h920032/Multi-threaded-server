...
int Ui::is_text(int x, int y)
{
	for(int i = 0; i < 16; i++)
	{
		...
		else if(i == 11 && is_login == 1 && text_list[i].y == y && text_list[i].x < x && text_list[i].x + text_list[i].contain.length() + to_string(money).length() > x)
		{
			return (-1);
		}
		else if(i == 12 && is_login == 1 && text_list[i].y == y && text_list[i].x < x && text_list[i].x + text_list[i].contain.length() + to_string(user_num).length() > x)
		{
			return (-1);
		}
	}
...
{
	if (is_login == 1 && x >= 31 && x < MAP_WIDTH && y > 4 && y <= 4 + user_num - 1)
	{
	...
	for (int i(0); i <= MAP_HEIGHT; i++) 
	{
        for (int j(0); j <= MAP_WIDTH; j++) 
		{
			...
			else if (is_text(j, i) != -2)
			{
				...
				if (is_text(j, i) == 11 && is_login == 1)
				{
					cout << money;
				}
				else if(is_text(j, i) == 12 && is_login == 1)
				{
					cout << user_num;
				}
			}
...
Ui::Ui()
{
	...
	text_list[8].x = 8;
	text_list[8].y = 22;
	text_list[8].contain = "[ ]TRANS NO.:";
	text_list[9].x = 8;
	text_list[9].y = 24;
	text_list[9].contain = "[ ]TRANS MONEY:";
	text_list[10].x = 28;
	text_list[10].y = 27;
	text_list[10].contain = "[ ]TRANS";
	text_list[11].x = 8;
	text_list[11].y = 12;
	text_list[11].contain = "Money:";
	text_list[12].x = 8;
	text_list[12].y = 14;
	text_list[12].contain = "Online User:";
	text_list[13].x = 31;
	text_list[13].y = 4;
	text_list[13].contain = "Name";
	text_list[14].x = 39;
	text_list[14].y = 4;
	text_list[14].contain = "IP";
	text_list[15].x = 55;
	text_list[15].y = 4;
	text_list[15].contain = "Port";
}
...
int Ui::enter()
{
	...
	switch(status)
	{
		...
		case 10:
			...
			{
				case 9:
					input = text_list[status-1].contain.substr(13);
					break;
				case 10:
					input = text_list[status-1].contain.substr(15);
					break;
			}
			char type_in;
			type_in = 0;
